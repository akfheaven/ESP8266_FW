...
static char ipInfoArr[20] = "192.168.1.111";
static int portInfo = 8999;
static char wifiName[32] = "ziroom501";
static char wifiPsw[32] = "4001001111";
...
{
	espconn_send(at_espconn_demo_espconn_ptr,"sendBack:",9);
...
at_espconn_demo_connect_cb(void *arg)
{
	...
	espconn_send(at_espconn_demo_espconn_ptr,"conncted",8);
}
...
static void ICACHE_FLASH_ATTR at_espconn_demo_init(void)
{
  ...
  //Test upd
  at_espconn_demo_espconn_ptr->type = ESPCONN_UDP;
  at_espconn_demo_espconn_ptr->state = ESPCONN_NONE;
  at_espconn_demo_espconn_ptr->proto.udp = (esp_udp *)os_zalloc(sizeof(esp_udp));
  at_espconn_demo_espconn_ptr->proto.udp->local_port = espconn_port();
  at_espconn_demo_espconn_ptr->proto.udp->remote_port = portInfo;

  ip = ipaddr_addr(ipInfoArr);
  os_memcpy(at_espconn_demo_espconn_ptr->proto.udp->remote_ip,&ip,sizeof(ip));
  ...
  espconn_create(at_espconn_demo_espconn_ptr);
...
//重置输出到UDP
static void ICACHE_FLASH_ATTR
my_rx(uint8* data,int32 len){
	espconn_send(at_espconn_demo_espconn_ptr,data,len);
	//at_port_print("hhh\n");
}
...
static void ICACHE_FLASH_ATTR
at_exeSetupTestParamter(uint8_t id, char *pPara)
{
	//skip '='
	++pPara;

	int32 ret = at_data_str_copy(ipInfoArr, &pPara, sizeof(ipInfoArr));
	if(ret < 0){
		at_port_print("error ip");
		return ;
	}
	at_port_print(ipInfoArr);

	//skip ','
	++pPara;
	int err = 0;
	ret = at_get_next_int_dec(&pPara, &portInfo, &err);
	if(err != 0 || ret == 0){
		at_response_error();
		return ;
	}
	...
	//skip ','
	++pPara;
	ret = at_data_str_copy(wifiName, &pPara, sizeof(wifiName));
	if(ret < 0){
		at_port_print("error wifiName");
		return ;
	}
	at_port_print(wifiName);

	//skip ','
	++pPara;
	ret = at_data_str_copy(wifiPsw, &pPara, sizeof(wifiPsw));
	if(ret < 0){
		at_port_print("error wifiPsw");
		return ;
	}
	at_port_print(wifiPsw);

	//init wifi
	wifi_station_set_auto_connect(0);
	struct	station_config	stationConf;
	stationConf.bssid_set	=	0;
	os_memcpy(&stationConf.ssid,	wifiName,	sizeof(wifiName));
	os_memcpy(&stationConf.password,	wifiPsw,	sizeof(wifiPsw));
	...
	//set uart -> udp
	at_register_uart_rx_intr(my_rx);
...
at_funcationType at_custom_cmd[] = {
	{"+CONNPC", 7, NULL, NULL, at_exeSetupTestParamter, NULL},
